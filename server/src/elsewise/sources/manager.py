...
class SourceManager:
    ...
    def discover(
        ...
        with self.database.transition_lock, self.database.transaction() as db:
            ...
            if running is not None and running.selected_source_id in replaced_source_ids:
                running.source_status = "waiting_for_source"
            ...
            if running is not None and running.selected_source_id is None:
                live = self._live_candidates(db)
                if len(live) == 1:
                    running.selected_source_id = source.id
                    running.source_status = self._session_source_status(source)
            ...
            if running is not None:
                emit_ui_event(db, "session.state", running.id, self._session_payload(running))
    ...
    def update_health(self, message: SourceHealth, *, paired_client_id: str) -> str:
        with self.database.transaction() as db:
            ...
            if running is not None and running.selected_source_id == source.id:
                next_status = self._session_source_status(source)
                session_changed = running.source_status != next_status
                running.source_status = next_status
                if session_changed:
                    emit_ui_event(db, "session.state", running.id, self._session_payload(running))
    ...
    def attach_for_session(self, session_id: str) -> SourceEpochRecord | None:
        with self.database.transition_lock, self.database.transaction() as db:
            ...
            if session.recording_status != "running":
                return None
            source = (
                db.get(CaptureSourceRecord, session.selected_source_id)
                if session.selected_source_id
                else None
            )
            if source is None:
                live = self._live_candidates(db)
            ...
            session.source_status = self._session_source_status(source)
            return epoch

    def select(self, session_id: str, source_id: str) -> SourceEpochRecord:
        with self.database.transition_lock, self.database.transaction() as db:
            session = db.get(SessionRecord, session_id)
            ...
            producer_epoch_id = str(source.sanitized_metadata.get("producer_epoch_id", "unknown"))
            epoch = self._ensure_epoch(db, session, source, producer_epoch_id)
            emit_ui_event(db, "session.state", session.id, self._session_payload(session))
            assert epoch is not None
            return epoch

    def selected_epoch(self, session_id: str) -> SourceEpochRecord | None:
        with self.database.transaction() as db:
            session = db.get(SessionRecord, session_id)
            if session is None or session.selected_source_id is None:
                return None
            return db.scalar(
                select(SourceEpochRecord)
                .where(
                    SourceEpochRecord.session_id == session_id,
                    SourceEpochRecord.source_id == session.selected_source_id,
                    SourceEpochRecord.state.in_(("starting", "running")),
                )
    ...
    async def finalize_epoch(
        ...
        *,
        ...
        result = await self.connections.command(
            client_id,
            payload,
        ...
        with self.database.transaction() as db:
            ...
            if epoch is not None and epoch.state == "starting":
                if result.get("result") == "started":
                    ...
                    if session is not None and session.selected_source_id == epoch.source_id:
                        session.source_status = "degraded"
                        emit_ui_event(
                            db, "session.state", session.id, self._session_payload(session)
                        )
    ...
    async def finalize_session(self, session_id: str, *, hard_budget_seconds: float) -> None:
        ...
        async def finalize(item: tuple[str, str, dict[str, Any]]) -> tuple[str, dict[str, Any]]:
    ...
    @staticmethod
    def _live_candidates(db: Session) -> list[CaptureSourceRecord]:
        return list(
            db.scalars(
                select(CaptureSourceRecord).where(
                    CaptureSourceRecord.connected.is_(True),
    ...
    def _ensure_epoch(
        ...
        if session is None or session.selected_source_id != source.id:
            return None
        current = db.scalar(
            select(SourceEpochRecord).where(
                SourceEpochRecord.source_id == source.id,
    ...
    @staticmethod
    def _session_source_status(source: CaptureSourceRecord) -> str:
    ...
    def _source_payload(
        ...
        client = db.get(PairedClientRecord, source.paired_client_id)
        source_ids = list(
            db.scalars(
        ...
        return cls.source_payload(
            source,
            epoch,
            client_display_name=client.display_name if client else "Browser extension",
            browser_family=client.browser_family if client else "browser",
